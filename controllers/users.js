...
const users ={
    async signUp (req, res, next){
        ...
        if(!email || !password || !confirmPassword || !name || !sex){
          ...
          return appError("400","欄位未填寫！",next);
        }
        //姓名至少兩個字元以上
        if( !validator.isLength(name, { min: 2})){
          // return next(appError("400", "暱稱須至少2個字元以上", next));
          return appError("400", "暱稱須至少2個字元以上", next);
        }
        // 密碼正確
        ...
        if (isEmailExist){
          ...
          return appError(400, "已有相同e-mail註冊", next );
        }
        // 加密密碼
    ...
    async signIn (req, res, next){
        ...
        if( !user){
          ...
          return appError(400, "您尚未註冊，請先註冊", next);
        }
        // 檢查密碼
    ...
    async updatePassword (req, res, next){
        ...
        if (!password.trim() || !confirmPassword.trim()){
          ...
          return appError(400, "密碼不得為空白",next);
        }
        ...
        if(!validator.isStrongPassword(password, {
          ...
          // return next(appError(400, "密碼須為8碼且包含至少1個以上大寫英文和1個數字"), next);
          return appError(400, "密碼須為8碼且包含至少1個以上大寫英文和1個數字", next);
        }
    ...
    async updateProfile (req, res, next){
      ...
      if(!name.trim() || !sex.trim() || !photo.trim()){
        ...
        return appError(400, "姓名、性別和上傳照片為必填!", next);
      }
